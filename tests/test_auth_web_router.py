...
class TestMultipleSessionLogin:
    ...
    def test_multiple_logins_use_redis_set(
        self, auth_test_client, auth_mock_session, test_user, mock_redis_client
    ):
    ...
    def test_logout_removes_only_current_session(
        self, auth_test_client, auth_mock_session, test_user, mock_redis_client
    ):
        ...
        with (
            patch("app.auth.web_router.redis.from_url", return_value=mock_redis_client),
    ...
    def test_session_limit_removes_oldest_when_exceeded(
        self, auth_test_client, auth_mock_session, test_user
    ):
    ...
    def test_session_validation_uses_sismember(
        self,
        auth_test_client,
        auth_mock_session,
        mock_auth_middleware_db,
        test_user,
        mock_redis_client,
    ):
        ...
        with (
            ...
            auth_test_client.get("/web-auth/login", follow_redirects=False)
