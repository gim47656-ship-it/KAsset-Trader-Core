...
# ---------------------------------------------------------------------------
# Portfolio v2 — crypto+upbit_live는 운영 provider가 없어 unavailable이다.
# ---------------------------------------------------------------------------
...
@pytest.mark.asyncio
async def test_portfolio_v2_crypto_upbit_live_is_explicitly_unavailable():
    """운영 중인 crypto 계좌 provider가 없으므로 upbit_live scope는 빈 성공이
    아니라 명시적 unavailable로 끝난다."""
    ...
    assert len(results) == 1
    assert results[0].freshness_status == "unavailable"
    assert "provider upbit is not operational" in str(results[0].errors_json)
