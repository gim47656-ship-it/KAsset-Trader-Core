...
def test_task_module_is_registered_without_recurring_schedule():
    import app.tasks as task_package

    assert mod in task_package.TASKIQ_TASK_MODULES
    labels = getattr(mod.live_reconcile_crypto_periodic, "labels", {}) or {}
    assert labels.get("schedule") is None
    assert not hasattr(mod, "live_reconcile_us_periodic")
