...
실측 결함(2026-08-28): 라우트와 백엔드 수집이 모두 정상인데도 앱 폴링이 전부
401 이었다. 허용목록 누락이 유일한 원인이었고, 라우트를 직접 호출하는 bare
FastAPI + `dependency_overrides` 테스트는 이 결함을 구조적으로 잡을 수 없다.
그래서 라우트 목록 자체를 계약으로 고정한다.
"""
...
def test_installed_routes_are_discovered() -> None:
    ...
    assert len(paths) > 20
    assert "/api/v1/market/stream" in paths
...
def test_stream_path_is_allowed() -> None:
...
def test_allowlist_stays_exact_and_rejects_generic_core_surface() -> None:
    ...
    assert not is_kasset_token_allowed_path("/api/v1/market/stream/extra")
