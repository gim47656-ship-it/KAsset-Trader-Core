...
class TestWholeProfileClosedWorld:
    def test_base_inventory_includes_readonly_advisors_and_is_exactly_119_tools(
        self,
    ) -> None:
        # Closed world: the count moves only with a reviewed addition/removal.
        # ROB-1303 added get_spike_attribution (read-only attribution reader),
        # and ROB-1309 added screen_stocks_enrich alongside
        # screen_stocks_snapshot. The removed KIS account-routing advisor must
        # not remain in the active exact-set canonical inventory.
        ...
        assert "suggest_order_account" not in KIWOOM_KR_BASE_PROFILE_TOOL_NAMES
