"""Paused TaskIQ auto-reconcile for Upbit crypto live orders.

Registered with the worker so operators can kick or externally schedule it,
but it carries no in-code ``schedule=`` label. Recurrence is owned by external
operator automations plus env gate flips after safety review. The explicit
market/broker pair prevents cross-provider scanning.
"""
