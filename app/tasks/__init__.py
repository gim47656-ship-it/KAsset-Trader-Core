from app.tasks import (
    analyst_consensus_snapshot_tasks,
    binance_demo_root_reservation_reconcile_tasks,
    crypto_pending_order_alert_tasks,
    daily_candles_tasks,
    daily_scan_tasks,
    dart_disclosure_ingestion_tasks,
    disclosure_summary_tasks,
    execution_ledger,
    google_news_rss_ingestion_tasks,
    intraday_order_review_tasks,
    invest_momentum_event_tasks,
    invest_screener_snapshot_tasks,
    investor_flow_snapshot_tasks,
    journal_counterfactual_tasks,
    journal_verdict_tasks,
    kasset_market_events_tasks,
    kasset_paper_automation_tasks,
    kis_live_reconcile_tasks,
    kis_mock_reconciliation_tasks,
    kr_candles_tasks,
    kr_symbol_universe_tasks,
    live_reconcile_tasks,
    market_quote_snapshot_tasks,
    market_valuation_snapshot_tasks,
    mock_roundtrip_journal_tasks,
    news_relevance_judgment_tasks,
    nhplug_symbol_master_tasks,
    order_proposal_expiry_tasks,
    paper_cohort_tasks,
    pending_orders,
    research_reports_ingest_tasks,
    research_run_refresh_tasks,
    sec_edgar_ingestion_tasks,
    telegram_callback_inbox_tasks,
    toss_live_reconcile_tasks,
    toss_manual_activity_tasks,
    toss_warnings_sync_tasks,
    upbit_symbol_universe_tasks,
    us_candles_tasks,
    us_symbol_universe_tasks,
    watch_expiry_tasks,
    watch_follow_up_tasks,
    watch_scan_tasks,
)

TASKIQ_TASK_MODULES = (
    analyst_consensus_snapshot_tasks,
    binance_demo_root_reservation_reconcile_tasks,
    crypto_pending_order_alert_tasks,
    daily_candles_tasks,
    daily_scan_tasks,
    dart_disclosure_ingestion_tasks,
    disclosure_summary_tasks,
    execution_ledger,
    google_news_rss_ingestion_tasks,
    intraday_order_review_tasks,
    invest_momentum_event_tasks,
    invest_screener_snapshot_tasks,
    investor_flow_snapshot_tasks,
    market_quote_snapshot_tasks,
    market_valuation_snapshot_tasks,
    mock_roundtrip_journal_tasks,
    nhplug_symbol_master_tasks,
    news_relevance_judgment_tasks,
    journal_verdict_tasks,
    journal_counterfactual_tasks,
    kasset_market_events_tasks,
    kasset_paper_automation_tasks,
    kis_live_reconcile_tasks,
    kis_mock_reconciliation_tasks,
    live_reconcile_tasks,
    watch_follow_up_tasks,
    watch_expiry_tasks,
    research_reports_ingest_tasks,
    research_run_refresh_tasks,
    sec_edgar_ingestion_tasks,
    watch_scan_tasks,
    kr_candles_tasks,
    kr_symbol_universe_tasks,
    order_proposal_expiry_tasks,
    pending_orders,
    paper_cohort_tasks,
    upbit_symbol_universe_tasks,
    us_candles_tasks,
    us_symbol_universe_tasks,
    telegram_callback_inbox_tasks,
    toss_live_reconcile_tasks,
    toss_manual_activity_tasks,
    toss_warnings_sync_tasks,
)
