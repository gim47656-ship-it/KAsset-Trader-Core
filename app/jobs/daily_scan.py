...
class DailyScanner:
    ...
    async def check_price_crash(
        ...
        logger.info(
            "Crash scan universe: rank_limit=%d ranked=%d",
            rank_limit,
            len(market_codes),
        )
        ...
        for ticker in tickers:
            ...
            threshold = self._crash_threshold_for_rank(rank)
            ...
            if abs_change_rate < threshold:
                ...
                if abs_change_rate >= threshold * near_miss_ratio:
                    near_miss_count += 1
                    logger.info(
                        "Crash scan near-miss market=%s rank=%s change=%+.2f%% threshold=%.2f%%",
                        market,
                        rank,
                        change_rate * 100,
                        threshold * 100,
            ...
            if not await self._should_alert(currency, "crash"):
                ...
                logger.info(
                    "Crash scan cooldown-skip market=%s rank=%s change=%+.2f%% threshold=%.2f%%",
                    market,
                    rank,
                    change_rate * 100,
                    threshold * 100,
            ...
            logger.info(
                "Crash alert accepted market=%s rank=%s change=%+.2f%% threshold=%.2f%%",
                market,
                rank,
                change_rate * 100,
                threshold * 100,
    ...
    async def check_sma20_crossings(
        ...
        market_codes = self._collect_krw_markets(
            top_coins=top_coins,
