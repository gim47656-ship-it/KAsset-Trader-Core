...
class CredentialVault:
    ...
    async def store_nh(
        ...
        *,
        ...
        async with self._write_lock:
            record = await self.record(db, owner_user_id, "NH", for_update=True)
    ...
    async def record(
        ...
        *,
        ...
        stmt = select(BrokerCredential).where(
            BrokerCredential.owner_user_id == owner_user_id,
    ...
    async def reveal_nh(
        ...
        record = await self.record(db, owner_user_id, "NH")
        if record is None:
    ...
    async def delete_nh(self, db: AsyncSession, owner_user_id: int) -> str | None:
        async with self._write_lock:
            record = await self.record(db, owner_user_id, "NH", for_update=True)
            if record is None:
                return None
