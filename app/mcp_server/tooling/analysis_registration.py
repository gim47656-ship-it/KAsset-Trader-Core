...
def register_analysis_tools(
    ...
    @mcp.tool(
        ...
        description=(
            "Analyze multiple stocks in parallel. quick=True is a DB-only fast projection "
            "with compact summaries; quick=False runs the full analysis. "
            "quick=True's current_price is the close of the most recent CLOSED daily "
            "candle read from the DB (data_state='stale', "
            "data_state_reason='db_only_projection') — it is NOT a live quote. Call "
            "get_quote whenever a live price or live session/NXT tradability state is "
            "required — including a live US regular-session price; use "
            "get_quote(include_extended_hours=True) for US premarket/afterhours, use "
            "get_quote for previous_close, and get_ohlcv when OHLC candles are required. "
            "The quick allowlist is symbol, market_type, source, current_price, latest OHLCV, "
            "RSI-14, top-three supports/resistances, compact decision_history/earnings, "
            "and the freshness envelope; it does "
            "not run news, profile, provider earnings, consensus, recommendation, or holdings work, "
            "and it does not carry live quote/session provenance (nxt_tradable*, price_source, "
            "session, session_state, krx_prev_close, change_pct, venue, quote_asof, delayed, "
            "price_data_state, fresh_artifact_exists) — call get_quote for those. "
            "Compact decision_history and earnings are read from batch SQL read models. "
            "The quick path reads all requested daily history with a bounded batch query "
            "(at most 12 DB executions including history/earnings) and makes zero HTTP requests. "
            "include_position is accepted for compatibility but has no effect for "
            "any quick value — analyze_stock_batch never attaches a position field "
            "(quick or full); use get_holdings for per-account positions. "
            "In the full path, slowly-changing provider data (KR naver "
            "valuation/opinions, US yfinance valuation/opinions + finnhub profile) "
            "uses the existing intraday fetch-layer cache and refresh=True bypasses "
            "that cache read (ROB-638)."
        ),
    )
