...
def register_route_request_tools(mcp: FastMCP) -> None:
    ...
    _ = mcp.tool(
        ...
        description=(
            ...
            "order_proposal_create is the only generic order-intent step in the "
            "standard sequence. The non-sequenced support_reserve_net_consume "
            "tool is a conditional buy helper that requires an explicit evidence "
            "packet and does not widen direct broker access. Telegram human "
            "approval is required by this generic route contract, proposal "
            "revalidation owns fresh "
