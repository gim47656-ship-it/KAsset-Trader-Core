...
async def _get_quote_impl(
    ...
    try:
        ...
        if market_type == "equity_us":
            payload.update({"success": False, "detail": str(exc)})
        return payload
...
def _register_market_data_tools_impl(mcp: FastMCP) -> None:
    ...
    @mcp.tool(
        ...
        description=(
            ...
            "price; always call get_quote for that. US regular-session 현재가가 "
            "기본이며 US premarket/afterhours는 include_extended_hours=True를 "
            "명시하세요. 이 옵션은 provider를 바꾸지 않습니다. previous_close도 "
            "이 응답에서 확인하고, 캔들 이력은 get_ohlcv를 사용하세요."
        ),
    )
    async def get_quote(
        symbol: str | int,
