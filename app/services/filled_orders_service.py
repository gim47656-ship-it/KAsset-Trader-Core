...
async def fetch_filled_orders(
    ...
    # Toss KR/US만 운영 체결 조회 경로다. 나머지 시장은 빈 성공으로 위장하지
    # 않고 명시적 오류로 남긴다(호출자가 errors를 검사한다).
    for market in sorted(market_set - {"kr", "us"}):
        all_errors.append(
            {
                "market": market,
                "error": f"provider for market '{market}' is not operational",
            }
        )
