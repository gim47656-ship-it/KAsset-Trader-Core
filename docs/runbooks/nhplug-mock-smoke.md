...
- 잔고와 시세 요청은 시작 시 configured `act_no` allowlist를 확인하고, 최초 전송과 토큰 refresh 재전송 각각의 `send` 직전에 다시 확인한다. 시세 본문에는 계좌번호가 없지만 같은 verified configured account를 반복 확인해 이중 판별 상태를 유지한다.
