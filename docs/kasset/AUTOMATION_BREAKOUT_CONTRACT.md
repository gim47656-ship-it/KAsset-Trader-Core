...
- 뉴스·공시 요약도 MCP 직결을 우선하고 direct API, OpenRouter 순으로 availability fallback한다. 일반 뉴스는 호출당 최대 5건을 묶고 UTC 일일 provider attempt 상한(기본 100)을 적용한다. OpenRouter fallback 모델은 공식 slug `z-ai/glm-5.3-flash`다.
