...
- **The new profile starts fail-closed.** Network transports require a non-empty `MCP_AUTH_TOKEN`, and with Kiwoom mock enabled startup requires complete mock credentials plus the exact `https://mockapi.kiwoom.com` base URL — naming the missing keys instead of deferring to the first tool call. `MCP_PROFILE=kiwoom` startup behavior is deliberately unchanged.
- **No order-path or deployment change.** KRX pinning, the rejected-exchange set, the `dry_run=False` + `confirm=True` double gate, and every place/cancel/modify body are reused untouched. The profile is deliberately not wired into `MCP_PROFILE_PORTS`/launchd: `docs/runbooks/kiwoom-kr-mcp.md` records the per-session start command and the evidence for that judgment (the array only feeds the ROB-831 release-path verification and would arm a deploy gate without starting anything, while a resident service would add an unmonitored `0.0.0.0` listener holding mock credentials).
...
- **Pre-send failures are separated from uncertain acceptance.** Client/configuration, OAuth, and request-build failures proven before HTTP dispatch return `not_submitted` without reconciliation; full-smoke modify preserves known lineage for that outcome, trusted local validation remains actionable, and only failures after a tracked mutation send begins use the non-retryable `acceptance_uncertain` contract.
