...
### Removed (broker surface reduced to Toss + KAsset PAPER; migration 0)
- **NH PLUG is gone from the runtime.** The `app/services/brokers/nhplug/` client/auth/gating package, the Android `nh_adapter`, the NH-only credential vault and its `/api/v1/brokers/{provider}/credential(s)` + `/verify` routes, the NH-backed `GET /api/v1/market/orderbook` REST route with its `OrderbookResponse`/`OrderbookLevel` schemas and token allowlist entry, the NH symbol-master service/job/task/CLI, and the NH mock smoke script and runbooks are deleted. The Toss `/api/v1/market/stream` orderbook path is untouched.
- **The Android broker catalog is `PAPER` and `TOSS` only.** `TOSS` is exposed as `LIVE_READ_ONLY` with no order capability and no app-side credential registration; order preview/submit/amend/cancel stay PAPER-only and every other provider stays fail-closed instead of falling through to PAPER. `Broker`/`SystemBrokerStatus` no longer carry the credential-vault fields (`credentialId`, `appKeyMasked`, `accountNoMasked`, `lastVerifiedAt`).
- **Equity orderbooks have no provider.** `market_data.get_orderbook` and the MCP `get_orderbook` tool serve the KRW crypto market only; KR equity requests are rejected instead of synthesized, and the invest stock-detail orderbook panel returns no snapshot for equities.
- **Historical data is preserved.** `kasset_broker_credentials`, `symbol_master`, the KIS/Upbit/Alpaca ledger tables, their rows, and every Alembic revision (including `20260828_nhplug_symbol_master`) are untouched; only runtime wiring was removed.
- **Every non-PAPER/Toss order provider is gone from the runtime.** The KIS, Kiwoom, Alpaca paper, Binance Spot/Futures/Demo-scalping, paper-cohort, and paper-evaluation surfaces — clients, transports, services, jobs, tasks, MCP registrars and tools, routers, schemas, smoke CLIs, and their runbooks — are deleted. `MCP_PROFILE` resolves only `default`, `crypto`, `db-paper`, `shadow-replay`, `analysis_readonly`, `account_read`, `tradingcodex_execution`, and `watch_repricing`; the `us-paper`, `kiwoom`, `kiwoom_kr`, and `paper_execution` profiles no longer exist.
- **Configuration carries no removed-provider credential, endpoint, or activation flag.** `Settings` drops every `kis_*`, `kiwoom_*`, and `alpaca_paper_*` field (credentials, base URLs, rate limits, OHLCV cache, circuit breaker, WebSocket transport), `binance_demo_scalping_enabled`, `BINANCE_DEMO_RESERVATION_RECONCILE_*`, `PAPER_EXECUTION_ENABLED`, `PAPER_COHORT_ENABLED`/`_CRON`, `us_dual_paper_preview_enabled`, `MOCK_ROUNDTRIP_JOURNAL_BRIDGE_ENABLED`, `LIVE_AUTO_RECONCILE_*`, the `crypto_pending_order_*` reminder settings, and the `validate_kis_mock_config` / `validate_kiwoom_*` / `validate_alpaca_paper_base_url` validators. `env.example`, `env.prod.example`, `deploy/kasset/{compose.yaml,env.example}`, and `scripts/setup-test-env.sh` no longer define or fabricate those keys.
- **Deployment no longer ships a WebSocket monitor.** `Dockerfile.ws`, the `docker-compose.prod.yml` `upbit_websocket` service, and its `ws-heartbeat` volume are removed because every monitor entrypoint (`websocket_monitor.py`, `kis_websocket_monitor.py`, `upbit_websocket_monitor.py`) is deleted. Toss fill confirmation stays polling-based.
- **CI drops only the provider-only gate.** The `kiwoom-dual-surface-smoke` job is removed with no surviving `needs` reference. `lint`, `test`, `taskiq-smoke`, `security`, `migration (PostgreSQL 15)`, `frontend`, and the `ci-required` aggregation are unchanged.

### Changed (KIS production cutover — Toss operational boundary; migration 0)
...
- **KIS configuration is gone, not dormant.** Core imports require no KIS credentials, and the transport/rate-limit/WebSocket/cache/circuit-breaker settings were removed together with the adapter modules. Deployment examples contain no KIS credentials or activation instructions. Historical KIS ledger models, rows, and `account_mode` values remain readable with their original provenance.
- **직접 실행·native KIS 표면도 제거했습니다.** KIS probe/mock/reconcile/consumer/selector 스크립트와 그 묘비 디렉터리(`scripts/_archive_kis/`)는 삭제됐고, KIS WebSocket 및 `mcp-paper_001` launchd plist/wrapper/profile은 배포에서 제거됩니다. Native healthcheck는 Upbit WebSocket만 확인합니다.
- **Toss fill evidence is polling-based.** Any accepted Toss live order requires a verified `toss_live.poll_fills_periodic` cadence of at most two minutes (`TOSS_FILL_POLL_ENABLED=true`, `TOSS_FILL_POLL_CRON=*/2 * * * *`) or immediate targeted non-dry reconcile.
...
### Changed (ROB-710 — `/invest` batch quotes are Toss-first; migration 0)
- **`/invest` current-price reads resolve Toss → snapshot.** `PriceFallbackResolver` ships a single `TOSS_FIRST_ORDER = ("toss", "snapshot")`: one ≤200-symbol Toss `MARKET_DATA` batch, then the snapshot tail. The flag-gated per-market flip and its `KIS_FIRST_ORDER` alternative were removed with the KIS provider, so there is no ordering flag left to flip. The `dict[str, float | None]` contract, per-layer fail-open, and the snapshot tail are unchanged.
