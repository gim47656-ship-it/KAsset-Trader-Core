...
- **2026-09-25 KR 일봉 백필 진행 중**: 백필 전 종목당 봉 수 중앙값이 23이었다. `scripts/backfill_daily_candles.py --market kr --all --include-benchmark --horizon-bars 400`을 일회성 컨테이너 `kasset-kr-backfill`(`compose run -d --rm`, 배포 재생성에 영향 없음)에서 돌린다. 진행은 `docker logs kasset-kr-backfill`. 스윙(1~2주)·장기(1~2개월) 전략 백테스트의 선행 조건이다.
