...
- **전략 데이터 범위**: KR 일봉 백필은 3,947종목(중앙값 400봉), 네이버 모바일 API 수급 백필은 3,944종목·950,074행으로 완료했다. 수급 `--days` 상한은 260이며 250일 백필은 `--batch-size 25`로 실행했다(100은 asyncpg 인자 32,767 한도 초과). 외국인 보유주수는 새 소스에 없어 None이고, 토스 공식 API에는 종목별 수급이 없다. DART는 아래 예약 작업으로 적재 중이다.
...
- **앱 검색 화면**: HANSE `361c341a`에 100건 요청·8행 가로 페이지가 커밋·push돼 있고 실기 설치까지 완료했다. 서버의 100건 상한이 남아 있으므로 무제한 전체 검색은 아니다. 공유용 APK는 QA 서명 `com.kasset.trader.debug`이며 앱 저장소 HANDOFF를 참조한다.
