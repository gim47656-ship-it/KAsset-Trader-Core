...
- **2026-09-25 KR 전략 데이터 백필**: 일봉은 `backfill_daily_candles.py --market kr --all --include-benchmark --horizon-bars 400`으로 완료(3,947종목, 중앙값 400봉, 250봉 이상 2,422종목). 수급은 `finance.naver.com/item/frgn.naver`가 Npay 증권 SPA로 바뀌어 수집이 전부 0행이 된 것을 PR #79(`1fe0d113`)에서 `m.stock.naver.com/api/stock/{code}/trend`(bizdate 역방향 페이징, 130거래일 이상 가능)로 교체한 뒤 `build_investor_flow_snapshots.py --all --days 60 --commit`을 재실행했다(외국인 보유주수는 새 소스에 없어 None). KIS는 이 운영 저장소에서 쓰지 않고 토스 공식 API에는 종목별 수급이 없다. DART 재무는 하루 한도 18,000건이라 `build_financial_fundamentals_snapshots.py --with-quarterly --skip-existing --limit 1500 --commit --allow-partial`을 하루 1회 수동으로 약 7일 이어서 돌린다(스케줄 등록 안 함). 모두 일회성 `compose run -d --rm` 컨테이너(`kasset-kr-flow`·`kasset-kr-dart`)다.
