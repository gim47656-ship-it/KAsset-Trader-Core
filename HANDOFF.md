...
갱신: 2026-09-02 (US 장중 ET-naive 분봉 수용, AI Hard Risk veto 제거(SHADOW 강등), 관문 funnel 계측 배포)
...
## 전체 진행 상태
- 운영 배포 이미지는 `kasset-trader-core:ee170aba581419f9033f3b609720cfcbcee58a42`다(브랜치 `fix/us-intraday-and-ai-veto`, 커밋 `5a5f737f` → `83f44174` → `ee170aba`). `api`, `worker`, `scheduler`, `mcp`, `ai-mcp` 5개 container가 이 태그로 기동 중이고 `/health`는 `ok`다. GitHub push와 PR은 아직 하지 않았다(서버 로컬 브랜치에만 있다).
...
## 다음 세션이 바로 할 일
1. 055550 PAPER 보유 4주는 Position Manager 청산 대상이다. 결정론 청산(`position_manager`/`position_exit`)이 정상 동작하는지 다음 KR 정규장에서 확인한다. 사용자 결정 없이 임의 청산하지 않는다.
